...
describe('World (ECS Orchestrator & Execution Pipeline)', () => {
  ...
  it('maintains deterministic execution across identical seeds', () => {
    ...
    expect(worldA.getPopulationSummary()).toEqual(worldB.getPopulationSummary());
    ...
    for (let i = 0; i < Math.min(10, worldA.pool.denseCount); i++) {
      ...
      expect(worldA.storage.positionsX[idxA]).toBeCloseTo(worldB.storage.positionsX[idxB]);
      expect(worldA.storage.positionsY[idxA]).toBeCloseTo(worldB.storage.positionsY[idxB]);
      expect(worldA.storage.energies[idxA]).toBeCloseTo(worldB.storage.energies[idxB]);
    }
  });
});
