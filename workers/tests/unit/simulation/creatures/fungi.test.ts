...
describe("simulation/creatures/fungi", () => {
  it("ignores dead matter outside perception radius", async () => {
    const fungus = buildFungus({
      position: { x: 0, y: 0 },
      perceptionRadius: 20,
    });
    const farDead = buildDeadMatter({
      id: "dead-far",
      position: { x: 100, y: 0 },
      energy: 50,
    });
  ...
  it("moves slowly toward dead matter within perception but outside decomposition range", async () => {
    ...
    const nearbyDead = buildDeadMatter({
      id: "dead-mid",
      position: { x: 40, y: 0 },
      energy: 50,
    });
  ...
  it("decomposes dead matter in range and gains energy", async () => {
    ...
    const deadMatter = buildDeadMatter({
      id: "dead-near",
      position: { x: 5, y: 0 },
      energy: 50,
    });
    ...
    expect(deadMatter.energy).toBe(24);
  });
  ...
  it("picks nearest dead matter when multiple are in range", async () => {
    ...
    const close = buildDeadMatter({
      id: "close",
      position: { x: 3, y: 0 },
      energy: 20,
    });
    const far = buildDeadMatter({
      id: "far",
      position: { x: 60, y: 0 },
      energy: 80,
    });
