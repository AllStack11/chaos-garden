...
export function createNewCarnivoreEntity(
  ...
  return {
    ...
    isAlive: true,
    type: 'carnivore',
    name: generateRandomName('carnivore', parentName),
    species: 'Stalkers',
    position: { ...position },
    energy: 50,
    health: 100,
 ...
 */
export function processCarnivoreBehaviorDuringTick(
  carnivore: Entity,
  ...
  if (targetPrey) {
    ...
    if (distance <= HUNTING_DISTANCE) {
      ...
      eventLogger.logDeath(targetPrey, `hunted by ${carnivore.species}`);
    } else {
      // Pursue prey
  ...
  if (carnivore.energy >= REPRODUCTION_THRESHOLD) {
    if (willRandomEventOccur(carnivore.reproductionRate)) {
      const child = attemptCarnivoreReproduction(carnivore, carnivore.gardenStateId ?? 0, eventLogger);
      if (child) {
 ...
 */
function attemptCarnivoreReproduction(
  parent: Entity,
  gardenStateId: number,
  eventLogger: EventLogger
): Entity | null {
...
export function isCarnivoreDead(carnivore: Entity): boolean {
  return carnivore.health <= 0 || carnivore.energy <= 0;
}
