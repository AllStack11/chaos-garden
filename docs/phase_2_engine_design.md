...
- [x] **Living Soil Grid** (`packages/engine/src/environment/SoilGrid.ts`)
  ...
  - [x] Unit tests in `packages/engine/tests/environment/SoilGrid.test.ts` (conservation of mass, CFL stability, boundary wrap, asymmetric buffer checks)
- [x] **Spatial Hash Grid** (`packages/engine/src/spatial/SpatialHashGrid.ts`)
  ...
  - [x] Implement neighbor query: `query(x, y, radius)` using pre-allocated `queryBuffer: Int32Array(32)` (zero heap allocations)
  - [x] Implement fast radial distance filtering within spatial buckets
  - [x] Unit tests in `packages/engine/tests/spatial/SpatialHashGrid.test.ts` (query accuracy, entity distribution, boundary cases, 32-slot buffer capacity)
...
- [x] **Perception & Craig Reynolds Steering** (`packages/engine/src/systems/SteeringSystem.ts`)
  - [x] Implement separation force ($\vec{F}_{\text{sep}} \propto 1/r^2$ from neighbors within personal space with toroidal deltas)
  - [x] Implement alignment force ($\bar{\vec{v}}_{\text{flock}} - \vec{v}$) and cohesion force (relative toroidal flock centroid displacement)
  - [x] Implement pursuit/seeking force towards nearest food/prey across toroidal wrap
  - [x] Implement evasion/fleeing force away from predators within `fleeRadius` across toroidal wrap
  ...
  - [x] Unit tests in `packages/engine/tests/systems/SteeringSystem.test.ts` (separation, fleeing, toroidal boundary wrap)
...
- [x] **World Orchestrator** (`packages/engine/src/ecs/World.ts`)
  ...
  - [x] Implement render frame getter / buffer transfer API for Web Worker consumption (`getRenderFrame`, `getTransferableRenderFrame`, `returnRenderBuffer`)
...
- [x] **Zero-Allocation Verification Test** (`packages/engine/tests/invariants/zeroAllocation.test.ts`)
  - [x] Run 1,000 continuous ticks under full load (2,000 entities) in Node.js with `--expose-gc`
...
- [x] **Full Test Suite & Type Check Execution**
  - [x] `npm run test -w @chaos-garden/engine`
  - [x] `npm run type-check -w @chaos-garden/engine`
  - [x] `npm run test:all`
